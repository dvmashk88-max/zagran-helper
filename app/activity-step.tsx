...
export function ActivityStep({items,onChange,focusGap}:{items:Activity[];onChange:(items:Activity[])=>void;focusGap?:string}){
 ...
 return <div>
  ...
  <div className="mt-6 space-y-3">{analysis.gaps.map(g=><div key={g.from} className="flex flex-col gap-3 rounded-2xl border border-amber-200 bg-amber-50 p-4 sm:flex-row sm:items-center sm:justify-between"><p>Не заполнено: <b>{monthLabel(g.from)} — {monthLabel(g.to)}</b>.</p><Button variant="outline" onClick={()=>reset(g.to)}>Заполнить этот период</Button></div>)}{analysis.overlaps.map(g=><div key={g.from} className="rounded-2xl border border-red-200 bg-red-50 p-4">Периоды пересекаются: <b>{monthLabel(g.from)} — {monthLabel(g.to)}</b>.</div>)}</div>
